// [NEXUS-CSOPS] Configuration & Global Core
// تم التعديل ليتوافق مع نظام الـ Modules في Production

window.SUPABASE_URL = "https://nrcnadkrnsjzbdzgrtgg.supabase.co";
window.SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5yY25hZGtybnNqemJkemdydGdnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1NDkzMjksImV4cCI6MjA5MDEyNTMyOX0.SLYEKj01VAbwnyEUNq6l2VUnfvoRs-zivplF01-oDLQ";
...
window.ChannelMgr = {
  ...
  sub: function (name, table, filter, cb) {
    this.unsub(name);
    try {
      var ch = window.sb.channel("nx_" + name);
  ...
  unsub: function (name) {
    try {
      if (this.channels[name]) {
        window.sb.removeChannel(this.channels[name]);
...
window.withRetry = function (fn, retries, delay) {
  retries = retries || 3;
  delay = delay || 800;
  function attempt(i) {
...
window.logAudit = function (action, page, actorId, targetId) {
  try {
    window.sb.from(window.DB.AUDIT_LOG).insert({
      action: action,
      page: page || null,
