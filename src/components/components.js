// [NEXUS-CSOPS] Shared Components & Analytics
// تم التعديل لضمان الوصول للمتغيرات العامة (window context)
...
window.Spinner = function Spinner(p) {
...
window.LoadingPage = function LoadingPage(p) {
  return React.createElement("div", {
    style: {
...
window.EmptyState = function EmptyState(p) {
  return React.createElement("div", {
    style: {
      display: "flex", flexDirection: "column", alignItems: "center",
...
window.PageHeader = function PageHeader(p) {
  return React.createElement("div", {
    style: {
...
window.NxAvatar = function NxAvatar(p) {
  var user = p.user;
  var size = p.size || "sm";
...
window.RoleBadge = function RoleBadge(p) {
  if (!p.role) return null;
  var color = window.RC.color[p.role] || "#94A3B8";
  var icon = window.RC.icon[p.role] || "🎧";
...
window.StatusBadge = function StatusBadge(p) {
  var s = window.STATUS_MAP[p.status] || { label: p.status || "Unknown", color: "#6B7280" };
  return React.createElement("span", {
    style: {
      display: "inline-flex", alignItems: "center", gap: 4,
...
window.PriorityBadge = function PriorityBadge(p) {
  var map = {
...
window.SearchInput = function SearchInput(p) {
...
window.OwnerAnalytics = function OwnerAnalytics(p) {
  ...
  function load() {
    ...
    window.withRetry(function () {
      return window.sb.from(window.DB.EMPLOYEES).select("id,is_online").eq("is_active", true);
    })
      .then(function (r) {
        var e = r.data || [];
      ...
      .then(function () {
        return window.withRetry(function () {
          return window.sb.from(window.DB.ATTENDANCE).select("id,status").eq("date", today);
        });
      })
      .then(function (r) {
        var a = r.data || [];
      ...
      .then(function () {
        return window.withRetry(function () {
          return window.sb.from(window.DB.BREAK_SCHEDULES).select("id").eq("date", today);
        });
      })
      .then(function (r) { result.breaksToday = (r.data || []).length }).catch(function () { })
      .then(function () {
        return window.withRetry(function () {
          return window.sb.from(window.DB.PERFORMANCE).select("tickets_handled,csat").gte("date", week);
        });
      })
      .then(function (r) {
        var perf = r.data || [];
  ...
  if (loading) return React.createElement(window.LoadingPage, { message: "Loading Analytics..." });
  if (!stats) return React.createElement(window.EmptyState, { icon: "👑", title: "No data" });
  var cards = [
  ...
  return React.createElement("div", { className: "nx-page-enter" },
    React.createElement(window.PageHeader, { title: "Owner Analytics", icon: "👑", subtitle: "Real-time overview" }),
