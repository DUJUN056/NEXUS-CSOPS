...
const RC = {
  ...
  isOwner(user) { return user?.role === "Owner"; },
  isMgr(user) {
    return ["Owner","Shift Leader","Team Leader"]
      .includes(user?.role);
  },
  isSME(user) { return user?.role === "SME"; },
  canAccess(user, page) {
    if (!user) return false;
    if (user.role === "Owner") return true;
    if (["Owner Analytics"].includes(page)) return false;
    if (["Break Management","Audit Log"].includes(page)) {
      return ["Owner","Shift Leader","Team Leader"]
        .includes(user.role);
    }
    return true;
...
const ThemeMgr = {
  get(user) {
    try { return localStorage.getItem("nx_theme") || "nika"; }
    catch(e) { return "nika"; }
  },
  set(themeId) {
    try {
...
const ThemeImageMgr = {
  set(theme, page, url) {
    try { localStorage.setItem("nx_img_"+theme+"_"+page, url); }
    catch(e) {}
  },
  getSync(theme, page) {
    try { return localStorage.getItem("nx_img_"+theme+"_"+page) || null; }
    catch(e) { return null; }
  },
  remove(theme, page) {
    try { localStorage.removeItem("nx_img_"+theme+"_"+page); }
    catch(e) {}
  },
  clear() {
    try {
      Object.keys(localStorage)
        .filter(function(k){ return k.indexOf("nx_img_")===0; })
...
const ChannelMgr = {
  ...
  unsubAll() {
    try {
      var self = this;
      Object.keys(this.channels).forEach(function(k){ self.unsub(k); });
    } catch(e) {}
  }
};
...
async function logAudit(action, details, actorId, targetId) {
  try {
    await sb.from("audit_log").insert({
      action: action, details: details||null,
      actor_id: actorId||null, target_id: targetId||null,
...
async function sendNotification(userId, type, title, body, link) {
  try {
    await sb.from("notifications").insert({
      user_id: userId, type: type||"system",
      title: title||"Notification", body: body||null,
      link: link||null, is_read: false,
      created_at: new Date().toISOString()
    });
  } catch(e) {}
}
...
function fmtDuration(seconds) {
  ...
  try {
    var s=Math.abs(Math.floor(seconds)),h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sec=s%60;
...
function adaptColor(color) { return color||"var(--primary)"; }
function getSaudiPeriod() {
  try {
    var h=parseInt(new Date().toLocaleString("en-US",{timeZone:"Asia/Riyadh",hour:"numeric",hour12:false}));
...
function ToastContainer() {
  var st=React.useState([]),toasts=st[0],setToasts=st[1];
  React.useEffect(function(){
    function handler(e){
      var id=Date.now()+Math.random(),d=e.detail;
      setToasts(function(p){return p.concat([{id:id,message:d.message,type:d.type}]);});
      setTimeout(function(){setToasts(function(p){return p.filter(function(x){return x.id!==id;});});},d.duration||3500);
    }
  ...
  return React.createElement("div",{style:{position:"fixed",bottom:24,right:24,zIndex:99999,display:"flex",flexDirection:"column",gap:8,maxWidth:360,pointerEvents:"none"}},
    toasts.map(function(x){
      return React.createElement("div",{key:x.id,style:{padding:"12px 16px",borderRadius:10,fontSize:13,fontWeight:600,color:"#fff",background:x.type==="success"?"#22C55E":x.type==="error"?"#EF4444":x.type==="warning"?"#EAB308":"#3B82F6",boxShadow:"0 4px 20px rgba(0,0,0,0.3)",display:"flex",alignItems:"center",gap:8,pointerEvents:"auto"}},
        React.createElement("span",null,x.type==="success"?"✅":x.type==="error"?"❌":x.type==="warning"?"⚠️":"ℹ️"),
        x.message);
    })
  );
}
...
function Tabs(p){
  ...
  return React.createElement("div",{className:"nx-tabs"},
    tabs.filter(Boolean).map(function(t){
      return React.createElement("button",{key:t.id,className:"nx-tab"+(p.active===t.id?" active":""),onClick:function(){p.onChange(t.id);}},t.label);
    })
  );
}

function NxAvatar(p){
  var user=p.user,size=p.size||"sm";
  var sizes={xs:24,sm:32,md:40,lg:56,xl:72};
  var px=sizes[size]||32,fs=Math.floor(px*0.4);
  ...
  if (user&&user.avatar_url){
    return React.createElement("img",{src:user.avatar_url,alt:name,style:{width:px,height:px,borderRadius:"50%",objectFit:"cover",flexShrink:0,border:"2px solid var(--border)"},onError:function(e){e.target.style.display="none";}});
  }
...
function RoleBadge(p){
  ...
  var color=RC.color[p.role]||"#94A3B8",icon=RC.icon[p.role]||"👤";
...
function PriorityBadge(p){
  var map={low:{label:"Low",color:"#22C55E",icon:"🟢"},medium:{label:"Medium",color:"#EAB308",icon:"🟡"},high:{label:"High",color:"#EF4444",icon:"🔴"},critical:{label:"Critical",color:"#7C3AED",icon:"🚨"}};
  var x=map[p.priority]||map.medium;
  return React.createElement("span",{style:{display:"inline-flex",alignItems:"center",gap:4,fontSize:10,fontWeight:700,color:x.color,background:x.color+"22",border:"1px solid "+x.color+"44",padding:"2px 6px",borderRadius:20}},x.icon+" "+x.label);
}

function SearchInput(p){
  return React.createElement("div",{style:{position:"relative",flex:1,minWidth:200}},
    React.createElement("span",{style:{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:14,pointerEvents:"none"}},"🔍"),
    React.createElement("input",{type:"text",className:"nx-input",placeholder:p.placeholder||"Search...",value:p.value||"",onChange:function(e){p.onChange(e.target.value);},style:{paddingLeft:36}})
  );
}

function TimerDisplay(p){
  if (p.seconds===null||p.seconds===undefined) return React.createElement("span",null,"—");
  var s=Math.abs(Math.floor(p.seconds)),m=Math.floor(s/60),sec=s%60;
...
function FileUploadBtn(p){
  ...
  return React.createElement("div",null,
    React.createElement("input",{ref:ref,type:"file",accept:p.accept||"*",style:{display:"none"},onChange:function(e){var f=e.target.files&&e.target.files[0];if(f)p.onFile(f);e.target.value="";}}),
    React.createElement("button",{className:"nx-btn nx-btn-secondary nx-btn-sm",onClick:function(){if(ref.current)ref.current.click();}},p.label||"Upload File")
  );
}
