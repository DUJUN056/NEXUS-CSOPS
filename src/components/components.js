...
/* ============================================================
   هذا الملف لا يحتوي على أي const أو var في أوله
   كل شيء يبدأ مباشرة بـ function
   ============================================================ */
...
function ToastContainer() {
  ...
  React.useEffect(function () {
    ...
    return function () {
      window.removeEventListener("nx-toast", handler);
    };
  }, []);
  ...
  async function loadStats() {
    try {
      ...
      setStats({
        ...
        online: emps.filter(function (e) {
          return e.is_online;
        }).length,
        suspended: emps.filter(function (e) {
          return e.is_suspended;
        }).length,
        todayAtt: att.length,
        onTime: att.filter(function (a) {
          return a.status === "on_time";
        }).length,
        late: att.filter(function (a) {
          return a.status === "late";
        }).length,
        activeBreaks: brks.filter(function (b) {
          return !b.end_time;
        }).length,
        exceededBreaks: brks.filter(function (b) {
          return b.status === "exceeded";
        }).length,
        totalCalls: kpis.reduce(function (s, k) {
          return s + (k.calls_handled || 0);
        }, 0),
        avgCsat: kpis.length > 0
          ? (kpis.reduce(function (s, k) {
              return s + (k.csat_score || 0);
            }, 0) / kpis.length).toFixed(1)
          : "—"
      });
    } catch (e) {
