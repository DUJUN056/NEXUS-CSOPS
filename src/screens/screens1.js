...
function UpdatesFeedPage(p){
  ...
  function load(){
    ...
    .catch(function(){showToast("Failed to load feed","error")})
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    showForm&&React.createElement("div",{className:"nx-card",style:{padding:20,marginBottom:20}},
      React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        ...
        React.createElement("button",{className:"nx-btn nx-btn-primary",onClick:submit},"Post Announcement")
      )
    ),
...
function SchedulePage(p){
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    items.length===0?React.createElement(EmptyState,{icon:"📅",title:"No schedule found",desc:"Your schedule will appear here"}):
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:8}},
      items.map(function(item){
        return React.createElement("div",{key:item.id,className:"nx-card nx-card-enter",style:{padding:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
          ...
          item.day_off
            ?React.createElement("span",{style:{color:"#22C55E",fontSize:13,fontWeight:700}},"🏖️ Day Off")
            :React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8}},
              React.createElement("span",{style:{fontSize:13,color:"var(--text-sub)",fontWeight:600}},(item.shift_start||"—")+" → "+(item.shift_end||"—"))
            )
        );
      })
    )
...
function AttendancePage(p){
  ...
  function checkIn(){
    ...
    .catch(function(){showToast("Failed to check in","error")});
  }
  function checkOut(){
    if(!today)return;
    ...
    .catch(function(){showToast("Failed to check out","error")});
  }
  ...
  return React.createElement("div",{className:"nx-page-enter"},
      ...
      React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}},
        React.createElement("div",null,
          ...
          React.createElement("div",{style:{fontSize:15,fontWeight:700,marginTop:4,color:"var(--text)"}},
            today
              ?(today.check_in?"In: "+fmtTime(today.check_in)+(today.check_out?" | Out: "+fmtTime(today.check_out):" | Still working"):"Not checked in")
              :"Not checked in"
          )
        ),
    ...
    items.length===0?React.createElement(EmptyState,{icon:"✅",title:"No attendance records"}):
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:8}},
      items.map(function(item){
...
function LiveFloorPage(p){
  ...
  var filtered=emps.filter(function(e){return !search||e.full_name.toLowerCase().indexOf(search.toLowerCase())>-1||(e.role||"").toLowerCase().indexOf(search.toLowerCase())>-1});
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    React.createElement("div",{className:"nx-grid-4",style:{marginBottom:20}},
      [{label:"Total",value:emps.length,icon:"👥",color:"var(--primary)"},
       ...
       {label:"Offline",value:emps.length-online,icon:"⚫",color:"#6B7280"},
       {label:"On Break",value:emps.filter(function(e){return e.status==="onbreak"}).length,icon:"☕",color:"#EAB308"}]
      .map(function(s){
