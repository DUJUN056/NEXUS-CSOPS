function AuditLogPage(p){
  ...
  function load(){
    withRetry(function(){
      return sb.from("audit_log")
        .select("*,actor:employees!actor_id(full_name,role)")
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    React.createElement("div",{style:{marginBottom:16}},
      React.createElement(SearchInput,{value:search,onChange:setSearch,placeholder:"Search..."})
    ),
    filtered.length===0
...
function ReportsNotesPage(p){
  ...
  var _4=React.useState({action:"",details:""});var form=_4[0];var setForm=_4[1];
  React.useEffect(function(){load()},[]);
  function load(){
    withRetry(function(){
      return sb.from("audit_log")
    ...
    showForm&&React.createElement("div",{className:"nx-card",style:{padding:20,marginBottom:16}},
      React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        ...
        React.createElement("textarea",{
          ...
          rows:3,
          value:form.details,
          onChange:function(e){setForm(function(f){return Object.assign({},f,{details:e.target.value})})}
        }),
...
function BreakManagementPage(p){
  ...
  React.useEffect(function(){
    ChannelMgr.sub("brkmgmt","breaks",null,load);
    return function(){ChannelMgr.unsub("brkmgmt")};
  },[]);
  function endBreak(id){
    withRetry(function(){
      return sb.from("breaks").update({
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    React.createElement(PageHeader,{title:"Break Management",icon:"⏱️",subtitle:active.length+" active today"}),
    React.createElement("div",{className:"nx-grid-4",style:{marginBottom:20}},
      [
        ...
        {label:"Exceeded",value:items.filter(function(b){return b.status==="exceeded"}).length,color:"#EAB308"}
      ].map(function(s){
    ...
    items.length===0
      ?React.createElement(EmptyState,{icon:"☕",title:"No breaks today"})
      :React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:8}},
          ...
          return React.createElement("div",{key:b.id,className:"nx-card",style:{padding:14}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}},
              ...
              React.createElement("div",{style:{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}},
                React.createElement("span",{style:{fontSize:12,color:"var(--text-sub)"}},
                  fmtTime(b.start_time)+" to "+(b.end_time?fmtTime(b.end_time):"Active")
                ),
                React.createElement(StatusBadge,{status:b.end_time?"completed":"active"}),
                !b.end_time&&RC.isMgr(user)&&React.createElement("button",{
                  className:"nx-btn nx-btn-secondary nx-btn-sm",
                  onClick:function(){endBreak(b.id)}
                },"End Break")
              )
            )
          );
