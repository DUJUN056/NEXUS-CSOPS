      ...
      items.map(function(b){
        return React.createElement("div",{key:b.id,className:"nx-card",style:{padding:12,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
          ...
          React.createElement("div",{style:{display:"flex",gap:10,fontSize:12,alignItems:"center"}},
            ...
            React.createElement("span",{style:{color:"var(--text-muted)"}},"to"),
...
function MyRequestsPage(p){
  ...
  var _4=React.useState({type:"leave",details:""});var form=_4[0];var setForm=_4[1];
  React.useEffect(function(){load()},[]);
  function load(){
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    React.createElement(PageHeader,{title:"My Requests",icon:"📋",subtitle:items.length+" requests",
      actions:React.createElement("button",{className:"nx-btn nx-btn-primary nx-btn-sm",onClick:function(){setShowForm(!showForm)}},showForm?"Cancel":"+ New")}),
...
function ShiftHandoverPage(p){
  ...
  var _2=React.useState(true);var loading=_2[0];var setLoading=_2[1];
  var _3=React.useState(false);var showForm=_3[0];var setShowForm=_3[1];
  var _4=React.useState({to_id:"",notes:""});var form=_4[0];var setForm=_4[1];
  ...
  function submit(){
    if(!form.to_id){showToast("Select recipient","warning");return}
    ...
    .then(function(){showToast("Submitted","success");setShowForm(false);setForm({to_id:"",notes:""});load()})
    .catch(function(){showToast("Failed","error")});
  }
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    showForm&&React.createElement("div",{className:"nx-card",style:{padding:20,marginBottom:16}},
      React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        ...
        React.createElement("textarea",{className:"nx-input",placeholder:"Notes...",rows:3,value:form.notes,onChange:function(e){setForm(function(f){return Object.assign({},f,{notes:e.target.value})})}}),
        React.createElement("button",{className:"nx-btn nx-btn-primary",onClick:submit},"Submit")
      )
    ),
          ...
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
            React.createElement("div",null,
              React.createElement("div",{style:{fontSize:13,fontWeight:700}},
                (item.from&&item.from.full_name)||"--"," to ",(item.to&&item.to.full_name)||"--"
              ),
...
function TTTrackerPage(p){
  ...
  var _2=React.useState(true);var loading=_2[0];var setLoading=_2[1];
  var _3=React.useState(false);var showForm=_3[0];var setShowForm=_3[1];
  var _4=React.useState({title:"",description:"",priority:"medium"});var form=_4[0];var setForm=_4[1];
...
function PerformancePage(p){
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    React.createElement(Tabs,{tabs:[{id:"day",label:"Today"},{id:"week",label:"7 Days"},{id:"month",label:"30 Days"}],active:period,onChange:setPeriod}),
    React.createElement("div",{className:"nx-grid-4",style:{marginBottom:20}},
      [{label:"Calls",value:totals.calls,color:"var(--primary)"},
    ...
    items.length===0?React.createElement(EmptyState,{icon:"?",title:"No KPI data"}):
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:8}},
      items.map(function(item){
        return React.createElement("div",{key:item.id,className:"nx-card",style:{padding:14}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
            ...
            React.createElement("div",{style:{display:"flex",gap:12,fontSize:12,flexWrap:"wrap"}},
              React.createElement("span",{style:{color:"var(--text-sub)"}},"Calls: "+(item.calls_handled||0)),
              React.createElement("span",{style:{color:"var(--text-sub)"}},"Resolved: "+(item.cases_resolved||0)),
              React.createElement("span",{style:{color:"var(--text-sub)"}},"CSAT: "+(item.csat_score||0)),
...
function QueuePage(p){
  ...
  React.useEffect(function(){load()},[]);
  function load(){
    withRetry(function(){return sb.from("employees").select("id,full_name,role,status,is_online,avatar_url").eq("is_active",true).eq("is_online",true).order("full_name")})
    .then(function(r){setItems(r.data||[])})
    .catch(function(){})
    .finally(function(){setLoading(false)});
  }
  React.useEffect(function(){
    ChannelMgr.sub("queue","employees",null,load);
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    React.createElement("div",{className:"nx-grid-4",style:{marginBottom:20}},
      [{label:"Online",value:items.length,color:"#22C55E"},
       {label:"Available",value:available.length,color:"var(--primary)"},
       {label:"Busy",value:busy.length,color:"#EF4444"},
       {label:"On Break",value:items.filter(function(e){return e.status==="onbreak"}).length,color:"#EAB308"}]
      .map(function(s){
        return React.createElement("div",{key:s.label,className:"nx-stat-card"},
          React.createElement("span",{className:"nx-stat-label"},s.label),
          React.createElement("div",{className:"nx-stat-value",style:{color:s.color,fontSize:22}},s.value)
...
function GamificationPage(p){
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    React.createElement("div",{className:"nx-card",style:{padding:20,marginBottom:16,textAlign:"center"}},
      ...
      React.createElement("div",{style:{fontSize:13,color:"var(--text-muted)",marginTop:4}},"Points and badges launching soon")
    ),
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:8}},
...
function SurveysPage(p){
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    items.length===0?React.createElement(EmptyState,{icon:"?",title:"No surveys",desc:"Surveys will appear here"}):
        ...
        return React.createElement("div",{key:item.id,className:"nx-card",style:{padding:16}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
            React.createElement("div",null,
              ...
              React.createElement("div",{style:{fontSize:12,color:"var(--text-muted)",marginTop:4}},"By "+((item.created_by&&item.created_by.full_name)||"--")+" "+fmtRelative(item.created_at))
            ),
