...
function startHeartbeat(userId){
  ...
  _hbInterval=setInterval(function(){
    sb.from("employees").update({is_online:true,last_seen:new Date().toISOString()}).eq("id",userId).then(function(){}).catch(function(){});
  },60000);
  window.addEventListener("beforeunload",function(){
    clearInterval(_hbInterval);
    sb.from("employees").update({is_online:false,status:"offline",last_seen:new Date().toISOString()}).eq("id",userId).then(function(){}).catch(function(){});
  });
}
...
function loadUserProfile(authId){
  return withRetry(function(){
    return sb.from("employees").select("*").eq("auth_id",authId).single();
  }).then(function(res){
...
function LoginPage(props){
  ...
  function handleLogin(e){
    ...
    sb.auth.signInWithPassword({email:email.trim(),password:password}).then(function(res){
      ...
      if(emp){props.onLogin(emp);showToast("Welcome "+(emp.full_name||""),"success")}
  ...
  return React.createElement("div",{style:{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",padding:24}},
    React.createElement("div",{style:{width:"100%",maxWidth:400,background:"var(--card)",border:"1px solid var(--border)",borderRadius:"var(--radius)",padding:40,boxShadow:"0 20px 60px rgba(0,0,0,0.5)"}},
      React.createElement("div",{style:{textAlign:"center",marginBottom:32}},
        ...
        React.createElement("p",{style:{fontSize:13,color:"var(--text-muted)",marginTop:4}},"CS Operations v4.2.0")
      ),
      React.createElement("form",{onSubmit:handleLogin,style:{display:"flex",flexDirection:"column",gap:16}},
        ...
        React.createElement("div",null,
          ...
          React.createElement("input",{type:"password",className:"nx-input",placeholder:"password",value:password,onChange:function(e){setPassword(e.target.value)},style:{fontSize:16}})
        ),
...
function App(){
  ...
  function handleLogout(){
    stopHeartbeat();
    if(user){
      sb.from("employees").update({is_online:false,status:"offline",last_seen:new Date().toISOString()}).eq("id",user.id).then(function(){}).catch(function(){});
    }
    ChannelMgr.unsubAll();
  ...
  if(loading){
    return React.createElement("div",{style:{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,background:"var(--bg)"}},
    ...
    React.createElement("div",{className:"nx-layout"},
      ...
      React.createElement("div",{className:"nx-main"},
        ...
        React.createElement("div",{className:"nx-content"},
          React.createElement(PageRouter,{page:page,user:user,setPage:handleSetPage})
        )
      )
    )
  );
...
(function(){
  try{
    ...
    if(!rootEl){console.error("[NEXUS-CSOPS] root not found");return}
    ReactDOM.createRoot(rootEl).render(React.createElement(App,null));
    console.log("[NEXUS-CSOPS] Started");
  }catch(e){
