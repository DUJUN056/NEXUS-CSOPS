// [NEXUS-CSOPS] Main Application Logic
...
function loadUserProfile(authId) {
  return window.withRetry(function () {
    ...
    if (!res || !res.data) {
      window.showToast("Profile not found. Contact admin.", "error");
      return null;
    }
    var emp = res.data;
    if (emp.is_suspended) {
    ...
    window.sb.from(window.DB.EMPLOYEES).update({
      is_online: true,
...
function PageRouter(props) {
  ...
  if (!user) return React.createElement(window.LoadingPage, { message: "Loading..." });
  
  if (!window.RC.canAccess(user, page)) {
  ...
  var pages = {
    "Updates Feed": window.UpdatesFeedPage,
    "Announcements": window.AnnouncementsPage,
    "Schedule": window.SchedulePage,
    "Attendance": window.AttendancePage,
    "Live Floor": window.LiveFloorPage,
    "My Break Schedule": window.MyBreakSchedulePage,
    "My Requests": window.MyRequestsPage,
    "Shift Handover": window.ShiftHandoverPage,
    "Case Handover": window.CaseHandoverPage,
    "TT Tracker": window.TTTrackerPage,
    "Performance": window.PerformancePage,
    "Queue": window.QueuePage,
    "Gamification": window.GamificationPage,
    "Surveys": window.SurveysPage,
    "Chat": window.ChatPage,
    "Notifications": window.NotificationsPage,
    "My Profile": window.MyProfilePage,
    "My Workspace": window.MyWorkspacePage,
    "Audit Log": window.AuditLogPage,
    "Reports & Notes": window.ReportsNotesPage,
    "Break Management": window.BreakManagementPage,
    "Owner Analytics": window.OwnerAnalytics
  };
...
function Sidebar(props) {
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", { className: "nx-sidebar-user" },
      React.createElement(window.NxAvatar, { user: user, size: "sm" }),
      React.createElement("div", { style: { flex: 1, minWidth: 0 } },
        ...
        React.createElement(window.RoleBadge, { role: user.role })
      )
    ),
...
function App() {
  ...
  React.useEffect(function () {
    ...
    var authListener = window.sb.auth.onAuthStateChange(function (event) {
      if (event === "SIGNED_OUT") {
        stopHeartbeat();
