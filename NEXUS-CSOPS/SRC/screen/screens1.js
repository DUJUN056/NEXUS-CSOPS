function UpdatesFeedPage(p){
  ...
  React.useEffect(function(){
    ChannelMgr.sub("feed","updates_feed",null,load);
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    items.length===0?React.createElement(EmptyState,{icon:"📭",title:"No updates yet",desc:"Updates will appear here"}):
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:16}},
      items.map(function(item){
        return React.createElement("div",{key:item.id,className:"nx-card nx-card-enter",style:{padding:20}},
          ...
          item.content&&React.createElement("p",{style:{fontSize:13,color:"var(--text-sub)",lineHeight:1.7,marginBottom:12}},item.content),
...
function AnnouncementsPage(p){
  ...
  var _4=React.useState({title:"",content:"",priority:"medium"});var form=_4[0];var setForm=_4[1];
  React.useEffect(function(){load()},[]);
  function load(){
    withRetry(function(){
      return sb.from("updates_feed")
  ...
  function submit(){
    ...
    withRetry(function(){
      return sb.from("updates_feed").insert({
        title:form.title,content:form.content||null,
        priority:form.priority,created_by:user.id,target_type:"all"
      });
    }).then(function(){
  ...
  function del(id){
    withRetry(function(){return sb.from("updates_feed").delete().eq("id",id)})
  ...
  return React.createElement("div",{className:"nx-page-enter"},
    ...
    showForm&&React.createElement("div",{className:"nx-card",style:{padding:20,marginBottom:20}},
      React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        ...
        React.createElement("textarea",{className:"nx-input",placeholder:"Content...",rows:3,value:form.content,onChange:function(e){setForm(function(f){return Object.assign({},f,{content:e.target.value})})}}),
        ...
        return React.createElement("div",{key:item.id,className:"nx-card",style:{padding:16}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}},
            React.createElement("div",{style:{flex:1}},
              ...
              item.content&&React.createElement("p",{style:{fontSize:13,color:"var(--text-sub)",lineHeight:1.6}},item.content),
  ...
  function load(){
    withRetry(function(){
      return sb.from("schedules")
        .select("*,shift:shift_types(name,start_time,end_time)")
      ...
      items.map(function(item){
        return React.createElement("div",{key:item.id,className:"nx-card nx-card-enter",style:{padding:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
          React.createElement("div",null,
            ...
            item.shift&&React.createElement("div",{style:{fontSize:12,color:"var(--text-muted)",marginTop:2}},item.shift.name)
          ),
          item.is_day_off
            ?React.createElement("span",{style:{color:"#22C55E",fontSize:13,fontWeight:700}},"Day Off")
            :React.createElement("span",{style:{fontSize:13,color:"var(--text-sub)",fontWeight:600}},item.shift?(item.shift.start_time+" to "+item.shift.end_time):"--")
        );
      })
    )
  );
...
function AttendancePage(p){
  ...
  function load(){
    withRetry(function(){
      return sb.from("attendance").select("*").eq("employee_id",user.id).order("date",{ascending:false}).limit(30);
    }).then(function(r){
      var rows=r.data||[];
      setItems(rows);
  ...
  function checkIn(){
    withRetry(function(){
      return sb.from("attendance").insert({employee_id:user.id,date:todayStr,check_in:new Date().toISOString(),status:"on_time"});
  ...
  function checkOut(){
    if(!today)return;
    withRetry(function(){
      return sb.from("attendance").update({check_out:new Date().toISOString()}).eq("id",today.id);
